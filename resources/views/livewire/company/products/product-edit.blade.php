<div>
    <flux:modal name="edit-product" class="min-w-[25rem] md:w-[50rem]">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Edit Product</flux:heading>
                <flux:text class="mt-2">Edit a Product.</flux:text>
            </div>
            ...
            <form wire:submit="update" class="flex flex-col gap-6">
                ...
                <flux:select  searchable placeholder="Pilih Category..." label="Category" wire:model.lazy="form.category_id">
                    {{-- <flux:select.option>Pilih Categories</flux:select.option> --}}
                    @foreach ($this->categories as $category)
                        <flux:select.option value="{{$category->id}}">{{$category->name}}</flux:select.option>
                    @endforeach
                </flux:select>
                <flux:input wire:model.lazy="form.price" label="Price" placeholder="Product Price" type="text"  />
